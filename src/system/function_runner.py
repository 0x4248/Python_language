...
def get_var(command):
    ...
    if command in interpreter.variables["names"]:
        return interpreter.variables["values"][interpreter.variables["names"].index(command)]
    # TODO: add raise
def run(command):
    if command.startswith("random"):
        if command.split(".")[1].startswith("int") and command.split(".")[1].startswith("intrange") == False:
